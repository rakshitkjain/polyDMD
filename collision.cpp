...
#include "collision.h" 
//Recalculation of "b" needed to make sure we are following the minimum image convention
...
//Thermostat per particle basis
int Collision::AndersenThermostat()
{
	int n;

	double net_mass=0.0, mass_ratio=0.0;
...
//Moving the thermostat particle to the actual position
...
void Collision::Bump(double L, int N)
{
	...
	Particle calculator;
//Move the system forward in time by TC.minimum_time
...
//Update cell list when you change cells
	...
	{
//Assigning colliding pair, moving just the colliding particles ahead to their true positions
